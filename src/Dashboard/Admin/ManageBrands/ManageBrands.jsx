...
const ManageBrands = () => {
  ...
  const onSubmit = (data) => {
    ...
    axiosPublic
      .post("/HomeBrand", formattedData) // Corrected POST endpoint
      .then(() => {
        Swal.fire({
          title: "Brand Added!",
  ...
  const handleDelete = async (itemId, itemName) => {
    try {
      ...
      if (result.isConfirmed) {
        await axiosPublic.delete(`/HomeBrand/${itemId}`);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-3/4 md:w-1/2 lg:w-1/3 text-black">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              {/* Image URL */}
              <div>
                <label className="font-semibold pb-2 text-lg">Image URL</label>
                <input
                  type="text"
              ...
              {/* Title */}
              <div>
                <label className="font-semibold pb-2 text-lg">Title</label>
                <input
                  type="text"
