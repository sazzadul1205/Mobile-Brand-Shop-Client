...
const ProductLaptop = () => {
  ...
  // Fetching Laptop Brands
  const {
    data: LaptopBrandsData = [],
    isLoading: LaptopBrandsIsLoading,
    error: LaptopBrandsError,
  } = useQuery({
    queryKey: ["LaptopBrands"],
    queryFn: () => {
      const productType = "Laptop";
      return axiosPublic
  ...
  // Fetching Laptop Conditions
  const {
    data: LaptopConditionsData = [],
    isLoading: LaptopConditionsIsLoading,
    error: LaptopConditionsError,
  } = useQuery({
    queryKey: ["LaptopConditions"],
    queryFn: () => {
      const productType = "Laptop";
      return axiosPublic
  ...
  // Extract unique filter options from LaptopProductsData
  const uniqueOperatingSystems = [
    ...new Set(LaptopProductsData.map((item) => item.operatingSystem)),
  ];

  // Use fetched brands for the dropdown
  const uniqueBrands = LaptopBrandsData.map((item) => item.brand);

  // Use fetched conditions for the dropdown
  const uniqueConditions = LaptopConditionsData.map((item) => item.condition);

  // Filtered products based on filters
  const filteredProducts = LaptopProductsData.filter((product) => {
  ...
  const [liked, setLiked] = useState(
    Array(LaptopProductsData.length).fill(false)
  );
