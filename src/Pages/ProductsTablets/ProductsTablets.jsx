...
const ProductsTablets = () => {
  ...
  // Extract unique filter options from TabletData
  const uniqueBrands = [...new Set(TabletData.map((item) => item.brand))];
  const uniqueConditions = [
    ...new Set(TabletData.map((item) => item.condition)),
  ];
  ...
  const filteredProducts = TabletData.filter((product) => {
  ...
  return (
    <div className="bg-gradient-to-b from-green-500 to-white py-24 text-black">
      <Helmet>
        <meta charSet="utf-8" />
        <title>Mobile Brand Shop || Products Tablet</title>
      </Helmet>
