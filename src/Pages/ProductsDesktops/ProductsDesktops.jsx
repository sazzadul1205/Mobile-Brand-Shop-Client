...
const ProductsDesktops = () => {
  ...
  // Fetching Desktop Brands
  const {
    data: DesktopBrandsData = [],
    isLoading: DesktopBrandsIsLoading,
    error: DesktopBrandsError,
  } = useQuery({
    queryKey: ["DesktopBrands"],
    queryFn: () => {
      const productType = "Desktop";
      return axiosPublic
  ...
  // Fetching Desktop Conditions
  const {
    data: DesktopConditionsData = [],
    isLoading: DesktopConditionsIsLoading,
    error: DesktopConditionsError,
  } = useQuery({
    queryKey: ["DesktopConditions"],
    queryFn: () => {
      const productType = "Desktop";
      return axiosPublic
  ...
  // Extract unique filter options from DesktopProductsData
  const uniqueOperatingSystems = [
    ...new Set(DesktopProductsData.map((item) => item.operatingSystem)),
  ];

  // Use fetched brands for the dropdown
  const uniqueBrands = DesktopBrandsData.map((item) => item.brand);

  // Use fetched conditions for the dropdown
  const uniqueConditions = DesktopConditionsData.map((item) => item.condition);

  // Filtered products based on filters
  const filteredProducts = DesktopProductsData.filter((product) => {
  ...
  const totalPages = Math.ceil(DesktopProductsData.length / ProductPerPage);
  ...
  const [liked, setLiked] = useState(
    Array(DesktopProductsData.length).fill(false)
  );
  ...
  if (
    ...
    DesktopBrandsIsLoading ||
    DesktopConditionsIsLoading
  ) {
    return <Loader />;
  }

  // Error state
  if (DesktopProductsError || DesktopBrandsError || DesktopConditionsError) {
    return (
  ...
  return (
    <div className="bg-gradient-to-b from-green-500 to-white lg:py-24 text-black pt-28 md:pt-16 lg:pt-11 lg:mt-14">
      <Helmet>
        <meta charSet="utf-8" />
        <title>Desktop Brand Shop || Products Desktop</title>
      </Helmet>
      {/* Search and Categories */}
      <TopSection />
      <div className="text-center mb-8 lg:mb-0">
        <p className="font-bold text-3xl">View Our Desktops</p>
      </div>
