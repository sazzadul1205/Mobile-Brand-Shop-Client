...
const ProductsMobile = () => {
  ...
  // Fetching Mobile Brands
  const {
    data: MobileBrandsData = [],
    isLoading: MobileBrandsIsLoading,
    error: MobileBrandsError,
  } = useQuery({
    queryKey: ["MobileBrands"],
    queryFn: () => {
      const productType = "Mobile";
      return axiosPublic
  ...
  // Fetching Mobile Conditions
  const {
    data: MobileConditionsData = [],
    isLoading: MobileConditionsIsLoading,
    error: MobileConditionsError,
  } = useQuery({
    queryKey: ["MobileConditions"],
    queryFn: () => {
      const productType = "Mobile";
      return axiosPublic
  ...
  // Extract unique filter options from MobileProductsData
  const uniqueOperatingSystems = [
    ...new Set(MobileProductsData.map((item) => item.operatingSystem)),
  ];

  // Use fetched brands for the dropdown
  const uniqueBrands = MobileBrandsData.map((item) => item.brand);

  // Use fetched conditions for the dropdown
  const uniqueConditions = MobileConditionsData.map((item) => item.condition);

  // Filtered products based on filters
  const filteredProducts = MobileProductsData.filter((product) => {
  ...
  const totalPages = Math.ceil(MobileProductsData.length / ProductPerPage);
  ...
  const [liked, setLiked] = useState(
    Array(MobileProductsData.length).fill(false)
  );
  ...
  if (
    ...
    MobileBrandsIsLoading ||
    MobileConditionsIsLoading
  ) {
    return <Loader />;
  }

  // Error state
  if (MobileProductsError || MobileBrandsError || MobileConditionsError) {
    return (
